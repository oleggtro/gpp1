...
int theworm_wormpos_y[WORM_LENGTH]; // Array defining y positions for worm elements
int theworm_wormpos_x[WORM_LENGTH]; // Array defining x positions for worm elements
...
void initializeColors()     {
  ...
  init_pair(COLP_USER_WORM, COLOR_RED, COLOR_BLACK); //@002
  init_pair(COLP_FREE_CELL, COLOR_BLACK, COLOR_BLACK);
  init_pair(COLP_USER_WORM_HEAD, COLOR, COLOR_BLACK);
} 
...
enum ResCodes doLevel(){
  ...
  bottomLeft_x = 0;
  res_code =
    initializeWorm(WORM_LENGTH, bottomLeft_y, bottomLeft_x, WORM_RIGHT, COLP_USER_WORM_HEAD);
...
enum ResCodes initializeWorm(int len_max, int headpos_y, int headpos_x, enum WormHeading dir, enum ColorPairs color) {
  ...
  theworm_wormpos_y[theworm_headindex] = headpos_y;//@005
...
int index = (theworm_headindex - 1); 
  ...
  placeItem(theworm_wormpos_y[theworm_headindex], theworm_wormpos_x[theworm_headindex], SYMBOL_WORM_HEAD,
      theworm_wcolor); //@007
  if(index == -1){ index = theworm_maxindex; }
  placeItem(theworm_wormpos_y[index], theworm_wormpos_x[index], SYMBOL_WORM_INNER_ELEMENT, theworm_wcolor); //??? differentiates between Head and inner element
}
void cleanWormTail() {
  int tailindex;  //  @006
  // Compute tailindex
...
void moveWorm(enum GameStates* agame_state) {
  ...
  headpos_y =  theworm_wormpos_y[theworm_headindex] + theworm_dy; //@012
